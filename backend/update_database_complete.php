...
echo "<h1>ERepair Complete Database Update</h1>";
...
try {
    ...
    $techniciansExists = $stmt->fetch();
    
    if (!$techniciansExists) {
        echo "<p>Creating technicians table...</p>";
        $db->exec("
        ...
        ");
        echo "<p style='color: green;'>✓ technicians table created</p>";
    } else {
        echo "<p style='color: green;'>✓ technicians table already exists</p>";
    }
    
    // Check if repair_shops table exists
    $stmt = $db->query("SHOW TABLES LIKE 'repair_shops'");
    $repairShopsExists = $stmt->fetch();
    
    if (!$repairShopsExists) {
        echo "<p>Creating repair_shops table...</p>";
        $db->exec("
        ...
        ");
        echo "<p style='color: green;'>✓ repair_shops table created</p>";
    } else {
        echo "<p style='color: green;'>✓ repair_shops table already exists</p>";
    }
    
    // Check if services table exists
    $stmt = $db->query("SHOW TABLES LIKE 'services'");
    $servicesExists = $stmt->fetch();
    ...
    if (!$servicesExists) {
        ...
        echo "<p style='color: green;'>✓ services table already exists</p>";
    }
    ...
    // Check if shop_services table exists
    $stmt = $db->query("SHOW TABLES LIKE 'shop_services'");
    $shopServicesExists = $stmt->fetch();
    
    if (!$shopServicesExists) {
        echo "<p>Creating shop_services table...</p>";
        $db->exec("
        ...
        ");
        echo "<p style='color: green;'>✓ shop_services table created</p>";
    } else {
        echo "<p style='color: green;'>✓ shop_services table already exists</p>";
    }
    ...
    echo "<ol>";
    echo "<li>✓ All database tables are now properly configured</li>";
    ...
    echo "<li>Test booking approval functionality</li>";
    echo "</ol>";
