...
function auth_shop_or_customer(PDO $db): array {
    $token = $_COOKIE['auth_token'] ?? '';
    if (!$token) {
        respond_json(401, ['error' => 'Not authenticated']);
    }
    $stmt = $db->prepare("SELECT u.id FROM users u INNER JOIN sessions s ON s.user_id = u.id WHERE s.token = ? AND s.expires_at > NOW()");
    $stmt->execute([$token]);
...
try {
    ...
    if (!isset($allowed[$mime])) {
        respond_json(400, ['error' => 'Invalid image type']);
    }
